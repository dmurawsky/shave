...
Brand.getInitialProps = async ({req, query}) => {
  if (!process.browser) {
    ...
    const snap = await database().ref('public/brands/' + query.id).once('value')
    const brand = snap.val()
    const productsSnap = await database().ref('public/products').orderByChild('brand').equalTo(query.id).once('value')
