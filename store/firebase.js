...
export const initFirebase = (dispatch, isServer) => {
  let uid = null
  if (apps.length === 0) {
        ...
        if (products) {
          ...
          if (!isServer) {
            auth().onAuthStateChanged(user => {
              if (user) {
                uid = user.uid
                database().ref(`users/${uid}/profile`).on('value', snap => {
                  const profile = snap.val()
                  dispatch({ type: 'LOAD_PROFILE', profile })
                })
                database().ref(`users/${uid}/cart`).on('value', snap => {
                  const cart = snap.val()
                  dispatch({ type: 'LOAD_CART', cart: formatCart(cart, products) })
                })
              } else {
                if (uid !== null) {
                  database().ref(`users/${uid}/profile`).off('value')
                  dispatch({ type: 'LOAD_PROFILE', profile: null })
                  database().ref(`users/${uid}/cart`).off('value')
